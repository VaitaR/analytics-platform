...
def test_polars_struct_fields():
    ...
    logger.info(f"🔄 Converted to Polars DataFrame")
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Modern struct.fields() successful, found {len(all_keys)} keys: {all_keys.to_list()}")
            ...
            try:
                ...
                if not sample_struct.is_empty():
                    ...
                    if first_row["decoded_props"] is not None:
                        ...
                        logger.info(f"✅ Fallback approach successful, found {len(all_keys)} keys: {all_keys}")
...
def test_json_schema_inference():
    ...
    logger.info(f"🧪 Testing schema inference with varying JSON structures")
    ...
    for approach_name, kwargs in approaches:
        ...
        try:
            ...
            # Try to get fields
            try:
                fields = decoded.select(pl.col("decoded").struct.fields()).to_series().explode().unique()
