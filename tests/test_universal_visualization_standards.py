...
from app import FunnelVisualizer, LayoutConfig, ColorPalette
from models import FunnelConfig, FunnelResults, CountingMethod, FunnelOrder, ReentryMode
...
class TestUniversalVisualizationStandards:
    ...
    def test_color_palette_accessibility(self, visualizer):
        ...
        semantic_colors = ['primary', 'secondary', 'success', 'warning', 'error', 'info', 'neutral']
    ...
    @pytest.mark.visualization
    @pytest.mark.parametrize("chart_type,data_size", [
    ...
    def test_chart_scaling_with_data_size(self, visualizer, chart_type, data_size):
        ...
        if chart_type == 'funnel':
            ...
            results = FunnelResults(
                ...
                conversion_rates=conversion_rates,
                drop_offs=[int(users_count[i] - users_count[i+1]) for i in range(len(users_count)-1)],
