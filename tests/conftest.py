...
class TestDataFactory:
    ...
    def create_funnel_data(spec: TestDataSpec, steps: List[str]) -> pd.DataFrame:
        ...
        if len(spec.conversion_rates) != step_count:
            raise ValueError(f"Conversion rates length ({len(spec.conversion_rates)}) must match steps length ({step_count})")
        ...
        for step_idx, step_name in enumerate(steps):
            ...
            for user_idx in range(users_for_step):
                ...
                events.append({
                    ...
                    'event_properties': json.dumps(event_props) if spec.include_properties else '{}',
                    'user_properties': json.dumps(user_props) if spec.include_properties else '{}'
                })
    ...
    def create_edge_case_data(case_type: str) -> pd.DataFrame:
        ...
        elif case_type == "duplicate_events":
        ...
        elif case_type == "missing_columns":
            return pd.DataFrame([{
...
def calculator_factory():
    ...
    def _create_calculator(config: FunnelConfig = None, use_polars: bool = True) -> FunnelCalculator:
        if config is None:
            config = FunnelConfig(
                conversion_window_hours=24,
                counting_method=CountingMethod.UNIQUE_USERS,
...
def assert_results_approximately_equal(result1: FunnelResults, result2: FunnelResults, tolerance: float = 0.01):
...
def log_test_performance(test_name: str, duration: float, data_size: int):
    ...
    logger.info(f"PERFORMANCE: {test_name} took {duration:.3f}s for {data_size} events "
