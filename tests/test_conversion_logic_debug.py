...
class TestConversionRateLogicBug:
    ...
    def test_debug_conversion_rate_calculation(self):
        ...
        print(f"- Users in step1: {test_data[test_data['event_name'] == 'step1']['user_id'].nunique()}")
        print(f"- Users in step2: {test_data[test_data['event_name'] == 'step2']['user_id'].nunique()}")
        ...
        print(f"   Manual verification: {overall_completed}/{overall_started} = {(overall_completed/overall_started*100):.2f}%")
        ...
        if not daily_results.empty:
            ...
            for idx, row in daily_results.iterrows():
                print(f"     {row['period_date']}: {row['started_funnel_users']} started, "
        ...
        if not daily_results.empty:
            ...
            print(f"   Timeseries sum:  {total_ts_started} started, {total_ts_completed} completed")
            ...
            if started_diff > 0 or completed_diff > 0:
                ...
                print(f"   This suggests the two methods are counting different users!")
    ...
    def _investigate_counting_difference(self, test_data, funnel_steps, calculator):
        ...
        for date in test_data['date'].unique():
            ...
            print(f"  {date}: {len(day_starters)} started, {len(day_completers)} completed, "
        ...
        print(f"  Cohort aggregate conversion: {total_cohort_completed/total_cohort_started*100:.2f}%")
        
        # The key insight: cohort logic might double-count or miss users!
        print(f"\n  POTENTIAL ISSUES:")
        print(f"  1. Users who start on multiple days are counted multiple times")
        print(f"  2. Users who start on one day but convert on another might be missed")
        print(f"  3. Conversion window enforcement differs between methods")
    ...
    def _test_user_reported_scenario(self):
        ...
        print(f"- Overall: 368 out of 42,458 = 0.9%")
        print(f"- Daily: 42.7% conversion rate")
        print(f"- This is impossible: 42,458 × 0.427 = 18,129 expected conversions")
        print(f"- But only 368 actual conversions")
        print(f"")
        print(f"POSSIBLE EXPLANATIONS:")
        print(f"1. Time series is using different data filtering")
        print(f"2. Conversion window logic differs between methods")
        print(f"3. Reentry mode handling creates double-counting")
        print(f"4. Period boundary handling causes users to be missed/duplicated")
        print(f"5. Different funnel step validation between methods")
