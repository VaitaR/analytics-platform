    ...
    for user_id in range(1, 31):
        ...
        for i, step in enumerate(steps):
            ...
            data.append({
                ...
                "event_name": step,
                "timestamp": event_time,
                "properties": str(properties)  # Convert dict to string for JSON-like structure
            })
...
class TestPolarsFallbackDetection:
    ...
    def test_detect_polars_expr_ambiguity_errors(self, events_data_base, log_capture):
        ...
        if errors_found:
            ...
            if any("the truth value of an Expr is ambiguous" in pattern for pattern, _ in errors_found):
                print("\n⚠️ WARNING: Polars expression ambiguity errors detected!")
                print("These errors indicate Python standard library functions are being used instead of Polars native expressions.")
    ...
    def test_object_dtype_errors(self, events_data_with_object_columns, log_capture):
        ...
        has_expr_ambiguity = re.search(expr_ambiguity_pattern, log_output, re.IGNORECASE) is not None
        has_fallback_to_standard = re.search(fallback_standard_join_pattern, log_output, re.IGNORECASE) is not None
        has_vectorized_fallback = re.search(vectorized_fallback_pattern, log_output, re.IGNORECASE) is not None
    ...
    def test_comprehensive_error_detection(self, events_data_with_object_columns, log_capture):
        ...
        test_configs = [
            ...
            {"order": FunnelOrder.ORDERED, "reentry": ReentryMode.FIRST_ONLY, "counting": CountingMethod.UNIQUE_USERS},
            
            # Config 2: Likely to trigger expression ambiguity errors
            {"order": FunnelOrder.ORDERED, "reentry": ReentryMode.OPTIMIZED_REENTRY, "counting": CountingMethod.UNIQUE_USERS},
            
            # Config 3: Tests unordered mode
            {"order": FunnelOrder.UNORDERED, "reentry": ReentryMode.FIRST_ONLY, "counting": CountingMethod.UNIQUE_USERS},
            
            # Config 4: Different counting method
            {"order": FunnelOrder.ORDERED, "reentry": ReentryMode.FIRST_ONLY, "counting": CountingMethod.UNIQUE_PAIRS}
        ]
        ...
        for i, cfg in enumerate(test_configs):
            ...
            has_dtype_error = any("could not extract number from any-value of dtype" in pattern for pattern, _ in errors_found)
            has_expr_ambiguity = any("the truth value of an Expr is ambiguous" in pattern for pattern, _ in errors_found)
        ...
        for config_key, errors in all_errors.items():
            for pattern, _ in errors:
                ...
                # Check if dtype errors are being handled properly with fallbacks
                    ...
                    has_fallback = any(re.search(fallback_pattern, log_capture.getvalue()) 
        ...
        if has_expr_ambiguity:
            ...
            print("These errors indicate Python standard library functions being used instead of Polars native expressions.")
            # Make the test fail only when expression ambiguity errors are found
            assert False, "Critical unhandled Polars expression ambiguity errors detected - fix required"
