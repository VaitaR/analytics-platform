...
def diagnose_cohort_attribution():
    ...
    print(results[['period_date', 'started_funnel_users', 'completed_funnel_users', 'conversion_rate']].to_string())
    print()
    ...
    for date, data in results_dict.items():
        print(f"{date}: {data['started_funnel_users']} starters, {data['completed_funnel_users']} completers, {data['conversion_rate']:.1f}% rate")
    ...
    if '2024-01-01' in results_dict and '2024-01-02' in results_dict:
        ...
        if jan_1['completed_funnel_users'] == 0 and jan_2['completed_funnel_users'] == 2:
            ...
            print(f"  Jan 1 should have 1 completer (user_A), but got {jan_1['completed_funnel_users']}")
            print(f"  Jan 2 should have 1 completer (user_B), but got {jan_2['completed_funnel_users']}")
            ...
            print(f"🤔 UNEXPECTED RESULT: Jan 1 = {jan_1['completed_funnel_users']}, Jan 2 = {jan_2['completed_funnel_users']}")
...
def test_pandas_vs_polars():
    ...
    # Force pandas
    try:
        pandas_result = calculator._calculate_timeseries_metrics_pandas(events_df, funnel_steps, '1d')
        print("📊 Pandas implementation result:")
        print(pandas_result[['period_date', 'started_funnel_users', 'completed_funnel_users', 'conversion_rate']].to_string())
    ...
    try:
        ...
        polars_result = calculator._calculate_timeseries_metrics_polars(polars_df, funnel_steps, '1d')
        print("\\n📊 Polars implementation result:")
        print(polars_result[['period_date', 'started_funnel_users', 'completed_funnel_users', 'conversion_rate']].to_string())
    except Exception as e:
