...
class TestBasicScenarios:
    def test_linear_funnel_calculation(
        self, calculator_factory, simple_linear_funnel_data, standard_funnel_steps
    ):
        """
        ...
        results = calculator.calculate_funnel_metrics(
            simple_linear_funnel_data, standard_funnel_steps
        )
    ...
    def test_funnel_with_noise_events(
        self, calculator_factory, simple_linear_funnel_data, standard_funnel_steps
    ):
        """
                ...
                [
                    {
                        ...
                        "timestamp": simple_linear_funnel_data.iloc[0]["timestamp"]
                        + timedelta(minutes=i),
                        "event_properties": json.dumps({}),
                        "user_properties": json.dumps({}),
                    },
    ...
    def test_funnel_steps_not_in_data(
        ...
        calculator = calculator_factory()
        results = calculator.calculate_funnel_metrics(
            simple_linear_funnel_data, non_existent_steps
        )
        ...
        assert (
            results.conversion_rates[0] == 100.0
