...
def test_comprehensive_ui_improvements():
    ...
    print(f"   Date range: {events_df['timestamp'].min().date()} to {events_df['timestamp'].max().date()}")
    ...
    for i, (_, row) in enumerate(results.head(sample_days).iterrows()):
        ...
        print(f"   🎯 COHORT: {row['started_funnel_users']} started → {row['completed_funnel_users']} completed → {row['conversion_rate']:.1f}% rate")
        print(f"   📊 DAILY: {row['daily_active_users']} active users, {row['daily_events_total']} total events")
        print(f"   📋 LEGACY: {row['total_unique_users']} unique users, {row['total_events']} events")
        
        # Verify cohort logic
        assert row['started_funnel_users'] > 0, f"Day {date} should have signups"
        assert row['conversion_rate'] >= 0 and row['conversion_rate'] <= 100, f"Conversion rate must be 0-100%, got {row['conversion_rate']}"
        
        # Verify daily metrics make sense
        assert row['daily_active_users'] >= row['started_funnel_users'], f"Daily active users should be >= cohort starters"
        assert row['daily_events_total'] >= row['daily_active_users'], f"Daily events should be >= daily users"
        
        # Verify backward compatibility
        assert row['total_unique_users'] == row['daily_active_users'], "Legacy metrics should match daily metrics"
        assert row['total_events'] == row['daily_events_total'], "Legacy events should match daily events"
        
        print()
    
    print(f"📊 AGGREGATE STATISTICS (First {sample_days} days):")
    aggregate_conversion = (total_cohort_completers / total_cohort_starters * 100) if total_cohort_starters > 0 else 0
    print(f"   Aggregate Cohort Conversion: {aggregate_conversion:.1f}% ({total_cohort_completers}/{total_cohort_starters})")
    print(f"   Total Daily Activity: {total_daily_users} user-days, {total_daily_events} event-days")
    print()
    ...
    print(f"   • Average Daily Conversion: {results['conversion_rate'].mean():.1f}% (arithmetic mean - less meaningful)")
    ...
    if abs(aggregate_rate - simple_avg) > 0.1:
        print(f"   ✅ Weighted vs Simple Average Difference: {abs(aggregate_rate - simple_avg):.1f}pp (shows proper calculation)")
...
def test_visualization_title_improvements():
    ...
    timeseries_data = calculator.calculate_timeseries_metrics(test_data, ['signup', 'purchase'], '1d')
    
    # Test dynamic chart titles
    test_cases = [
        ("started_funnel_users", "conversion_rate", "Users Starting Funnel (Cohort)", "Cohort Conversion Rate (%)"),
        ("daily_active_users", "conversion_rate", "Daily Active Users", "Cohort Conversion Rate (%)"),
    ]
    ...
    for primary_metric, secondary_metric, primary_display, secondary_display in test_cases:
        ...
        assert expected_title in actual_title or "Time Series" in actual_title, f"Chart should have meaningful title, got: {actual_title}"
