...
def test_daily_metrics_vs_cohort_metrics():
    ...
    for _, row in results.iterrows():
        ...
        print(f"{date}: {row['started_funnel_users']} starters, {row['completed_funnel_users']} completers")
        print(f"  Cohort conversion: {row['conversion_rate']:.1f}%")
        print(f"  Daily metrics: {row['total_unique_users']} unique users, {row['total_events']} events")
        print()
    ...
    for date, expected in expected_cohort_metrics.items():
        if date in results_dict:
            ...
            if actual['started_funnel_users'] != expected['starters']:
                cohort_errors.append(f"{date}: Expected {expected['starters']} starters, got {actual['started_funnel_users']}")
            if actual['completed_funnel_users'] != expected['completers']:
                cohort_errors.append(f"{date}: Expected {expected['completers']} completers, got {actual['completed_funnel_users']}")
    ...
    for date, expected in expected_daily_metrics.items():
        if date not in results_dict:
            daily_errors.append(f"{date}: Missing from results but had {expected['users']} active users")
        else:
            actual = results_dict[date]
            # The issue: we should have daily_active_users and daily_events_total as NEW metrics
            if 'daily_active_users' not in actual:
                daily_errors.append(f"Missing 'daily_active_users' metric")
            if 'daily_events_total' not in actual:
                daily_errors.append(f"Missing 'daily_events_total' metric")
...
def test_current_total_users_attribution():
    ...
    for _, row in results.iterrows():
        date = row['period_date'].strftime('%Y-%m-%d')
        print(f"{date}: total_unique_users = {row['total_unique_users']}, total_events = {row['total_events']}")
    ...
    if results_dict['2024-01-01']['total_unique_users'] != 1:
        issues.append(f"Jan 1 should have 1 unique user, got {results_dict['2024-01-01']['total_unique_users']}")
    if results_dict['2024-01-02']['total_unique_users'] != 2:
        issues.append(f"Jan 2 should have 2 unique users, got {results_dict['2024-01-02']['total_unique_users']}")
