...
class TestTimeSeriesCalculationComprehensive:
    ...
    def conversion_window_test_data(self):
        ...
        events.extend([
            {'user_id': 'within_window', 'event_name': 'Start', 'timestamp': base_date,
             'event_properties': '{}', 'user_properties': '{}'},
            {'user_id': 'within_window', 'event_name': 'Finish', 'timestamp': base_date + timedelta(minutes=30),
             'event_properties': '{}', 'user_properties': '{}'}
        ])
        
        # User who converts outside window
        events.extend([
            {'user_id': 'outside_window', 'event_name': 'Start', 'timestamp': base_date,
             'event_properties': '{}', 'user_properties': '{}'},
            {'user_id': 'outside_window', 'event_name': 'Finish', 'timestamp': base_date + timedelta(hours=2),
             'event_properties': '{}', 'user_properties': '{}'}
        ])
        
        # User who starts but never finishes
        events.append({
            'user_id': 'never_finishes', 'event_name': 'Start', 'timestamp': base_date,
            'event_properties': '{}', 'user_properties': '{}'
        })
    ...
    def edge_case_data(self):
        ...
        events.extend([
            {'user_id': 'boundary_user', 'event_name': 'Start', 'timestamp': base_date,
             'event_properties': '{}', 'user_properties': '{}'},
            {'user_id': 'boundary_user', 'event_name': 'End', 'timestamp': base_date + timedelta(seconds=30),
             'event_properties': '{}', 'user_properties': '{}'}
        ])
        
        # Single event user
        events.append({
            'user_id': 'single_event', 'event_name': 'Start', 'timestamp': base_date,
            'event_properties': '{}', 'user_properties': '{}'
        })
        ...
        events.extend([
            {'user_id': 'reverse_order', 'event_name': 'End', 'timestamp': future_time + timedelta(minutes=30),
             'event_properties': '{}', 'user_properties': '{}'},
            {'user_id': 'reverse_order', 'event_name': 'Start', 'timestamp': future_time,
             'event_properties': '{}', 'user_properties': '{}'}
        ])
    ...
    def test_precise_cohort_calculation(self, standard_calculator, precise_test_data):
        ...
        assert day1_row['started_funnel_users'] == 10, f"Day 1: Expected 10 starters, got {day1_row['started_funnel_users']}"
        assert day1_row['completed_funnel_users'] == 3, f"Day 1: Expected 3 completers, got {day1_row['completed_funnel_users']}"
        expected_day1_rate = (3 / 10) * 100  # 30%
        assert abs(day1_row['conversion_rate'] - expected_day1_rate) < 0.01, \
            f"Day 1: Expected {expected_day1_rate}% conversion, got {day1_row['conversion_rate']}%"
        
        # Validate step-by-step counts for Day 1
        assert day1_row['Sign Up_users'] == 10, f"Day 1: Expected 10 Sign Up users, got {day1_row['Sign Up_users']}"
        assert day1_row['Verify Email_users'] == 7, f"Day 1: Expected 7 Verify Email users, got {day1_row['Verify Email_users']}"
        assert day1_row['Complete Profile_users'] == 5, f"Day 1: Expected 5 Complete Profile users, got {day1_row['Complete Profile_users']}"
        assert day1_row['Purchase_users'] == 3, f"Day 1: Expected 3 Purchase users, got {day1_row['Purchase_users']}"
        
        # Validate Day 2 metrics (5 started, 2 completed)
        day2_row = result.iloc[1]
        assert day2_row['started_funnel_users'] == 5, f"Day 2: Expected 5 starters, got {day2_row['started_funnel_users']}"
        assert day2_row['completed_funnel_users'] == 2, f"Day 2: Expected 2 completers, got {day2_row['completed_funnel_users']}"
        expected_day2_rate = (2 / 5) * 100  # 40%
        assert abs(day2_row['conversion_rate'] - expected_day2_rate) < 0.01, \
        ...
        assert abs(day1_row['Sign Up_to_Verify Email_rate'] - day1_signup_to_verify) < 0.01, \
            f"Day 1: Sign Up to Verify rate should be {day1_signup_to_verify}%, got {day1_row['Sign Up_to_Verify Email_rate']}%"
        assert abs(day1_row['Verify Email_to_Complete Profile_rate'] - day1_verify_to_profile) < 0.01, \
            f"Day 1: Verify to Profile rate should be ~{day1_verify_to_profile:.2f}%, got {day1_row['Verify Email_to_Complete Profile_rate']}%"
        assert abs(day1_row['Complete Profile_to_Purchase_rate'] - day1_profile_to_purchase) < 0.01, \
    ...
    def test_conversion_window_enforcement(self, short_window_calculator, conversion_window_test_data):
        ...
        assert day_row['started_funnel_users'] == 3, f"Expected 3 starters, got {day_row['started_funnel_users']}"
        assert day_row['completed_funnel_users'] == 1, f"Expected 1 completer (within window), got {day_row['completed_funnel_users']}"
        
        expected_rate = (1 / 3) * 100  # ~33.33%
        assert abs(day_row['conversion_rate'] - expected_rate) < 0.01, \
        ...
        assert day_row['Finish_users'] == 1, f"Expected 1 Finish user (within window), got {day_row['Finish_users']}"
    ...
    def test_hourly_aggregation_accuracy(self, standard_calculator, multi_period_data):
        ...
        assert 48 <= len(result) <= 49, f"Expected 48-49 hours of data (hour boundaries), got {len(result)}"
        ...
        assert expected_step2_min <= total_step2 <= expected_step2_max, \
            f"Step2 users should be 75-85% of starters ({expected_step2_min}-{expected_step2_max}), got {total_step2}"
        
        assert expected_step3_min <= total_step3 <= expected_step3_max, \
        ...
        assert (result['conversion_rate'] >= 0).all(), "All conversion rates should be non-negative"
    ...
    def test_daily_aggregation_consistency(self, standard_calculator, multi_period_data):
        ...
        assert starters_diff <= 20, f"Daily and hourly starters should be close: daily={total_daily_starters}, hourly={total_hourly_starters}, diff={starters_diff}"
        
        # Validate that conversion rates are within reasonable range
        for i, day_row in daily_result.iterrows():
            assert 0 <= day_row['conversion_rate'] <= 100, f"Day {i}: Conversion rate {day_row['conversion_rate']} should be 0-100%"
            assert day_row['started_funnel_users'] >= day_row['completed_funnel_users'], \
                f"Day {i}: Starters should >= completers"
        
        print(f"✅ Daily aggregation consistency test passed: {len(daily_result)} days, daily_starters={total_daily_starters}, hourly_starters={total_hourly_starters}")
    ...
    def test_empty_data_handling(self, standard_calculator):
        ...
        empty_df = pd.DataFrame(columns=['user_id', 'event_name', 'timestamp', 'event_properties', 'user_properties'])
    ...
    def test_mathematical_consistency(self, standard_calculator, precise_test_data):
        ...
        for _, row in result.iterrows():
            ...
            for i in range(len(funnel_steps) - 1):
                ...
                assert next_step_users <= current_step_users, \
                    f"Step {i+1} users ({next_step_users}) should not exceed step {i} users ({current_step_users})"
            
            # Test conversion rate calculation
            if row['started_funnel_users'] > 0:
                calculated_rate = (row['completed_funnel_users'] / row['started_funnel_users']) * 100
                assert abs(row['conversion_rate'] - calculated_rate) < 0.01, \
            ...
            for i in range(len(funnel_steps) - 1):
                ...
                if from_users > 0:
                    ...
                    assert abs(row[rate_col] - expected_rate) < 0.01, \
                        f"Step rate mismatch for {rate_col}: stored {row[rate_col]}%, calculated {expected_rate}%"
                else:
                    assert row[rate_col] == 0.0, f"Rate should be 0 when no users in from step"
    ...
    def test_polars_pandas_consistency(self, precise_test_data):
        ...
        # Compare key metrics
        assert len(polars_result) == len(pandas_result), "Results should have same number of periods"
    ...
    def test_performance_with_large_dataset(self, standard_calculator):
        ...
        assert calculation_time < 30.0, f"Calculation took too long: {calculation_time:.2f} seconds"
