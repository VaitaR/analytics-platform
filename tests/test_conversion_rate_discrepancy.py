...
class TestConversionRateDiscrepancy:
    ...
    def test_overall_vs_timeseries_conversion_rates(self, diagnostic_data, calculator):
        ...
        print(f"- Time range: {diagnostic_data['timestamp'].min()} to {diagnostic_data['timestamp'].max()}")
        ...
        if not timeseries_results.empty:
            ...
            for idx, row in timeseries_results.iterrows():
                ...
                print(f"     {period}: started={started}, completed={completed}, "
        ...
        if not timeseries_results.empty:
            ...
            started_col = 'started_funnel_users' if 'started_funnel_users' in timeseries_results.columns else 'started_count'
            completed_col = 'completed_funnel_users' if 'completed_funnel_users' in timeseries_results.columns else 'completed_count'
            
            if started_col in timeseries_results.columns and completed_col in timeseries_results.columns:
                ...
                if conversion_diff > 10:
                    ...
                    print("This suggests different calculation methodologies between overall and timeseries metrics.")
            else:
    ...
    def test_conversion_window_impact(self, diagnostic_data, calculator):
        ...
        for window_hours in [1, 6, 24, 168]:  # 1h, 6h, 24h, 1 week
            ...
            overall_conversion = overall_results.conversion_rates[-1] if overall_results.conversion_rates else 0
            
            # Timeseries  
            timeseries_results = test_calculator.calculate_timeseries_metrics(diagnostic_data, steps, '1h')
            ...
            print(f"Window {window_hours}h: Overall={overall_conversion:.1f}%, Timeseries={ts_conversion:.1f}%, "
    ...
    def test_reentry_mode_impact(self, diagnostic_data):
        ...
        for reentry_mode in [ReentryMode.FIRST_ONLY, ReentryMode.OPTIMIZED_REENTRY]:
            ...
            print(f"{reentry_mode.value}: Overall={overall_conversion:.1f}%, Timeseries={ts_conversion:.1f}%, "
                  f"Diff={abs(overall_conversion - ts_conversion):.1f}%")
