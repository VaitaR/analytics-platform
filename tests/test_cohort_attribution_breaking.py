...
class TestCohortAttributionError:
    ...
    def test_cross_period_conversion_attribution(self):
        ...
        assert jan_1_data['started_funnel_users'] == 1, f"Jan 1 should have 1 starter, got {jan_1_data['started_funnel_users']}"
        assert jan_1_data['completed_funnel_users'] == 1, f"Jan 1 cohort should have 1 converter, got {jan_1_data['completed_funnel_users']}"
        assert abs(jan_1_data['conversion_rate'] - 100.0) < 0.01, f"Jan 1 cohort conversion should be 100%, got {jan_1_data['conversion_rate']}"
        ...
        assert jan_2_data['started_funnel_users'] == 10, f"Jan 2 should have 10 starters, got {jan_2_data['started_funnel_users']}"
        assert jan_2_data['completed_funnel_users'] == 0, f"Jan 2 cohort should have 0 converters, got {jan_2_data['completed_funnel_users']}"
        assert abs(jan_2_data['conversion_rate'] - 0.0) < 0.01, f"Jan 2 cohort conversion should be 0%, got {jan_2_data['conversion_rate']}"
        
        print("✅ COHORT ATTRIBUTION TEST PASSED: Conversions correctly attributed to cohort start dates!")
    ...
    def test_multi_day_conversion_window_attribution(self):
        ...
        assert jan_1['started_funnel_users'] == 5, f"Jan 1 cohort should have 5 starters, got {jan_1['started_funnel_users']}"
        assert jan_1['completed_funnel_users'] == 2, f"Jan 1 cohort should have 2 converters, got {jan_1['completed_funnel_users']}"
        expected_conversion_1 = 40.0
        assert abs(jan_1['conversion_rate'] - expected_conversion_1) < 0.01, f"Jan 1 conversion should be {expected_conversion_1}%, got {jan_1['conversion_rate']}"
        
        # Jan 2 cohort: 8 starters, 3 converters (37.5% conversion)
        jan_2 = results_dict['2024-01-02']
        assert jan_2['started_funnel_users'] == 8, f"Jan 2 cohort should have 8 starters, got {jan_2['started_funnel_users']}"
        assert jan_2['completed_funnel_users'] == 3, f"Jan 2 cohort should have 3 converters, got {jan_2['completed_funnel_users']}"
        expected_conversion_2 = 37.5
        assert abs(jan_2['conversion_rate'] - expected_conversion_2) < 0.01, f"Jan 2 conversion should be {expected_conversion_2}%, got {jan_2['conversion_rate']}"
        
        # Jan 3 cohort: 6 starters, 0 converters (0% conversion)
        jan_3 = results_dict['2024-01-03']
        assert jan_3['started_funnel_users'] == 6, f"Jan 3 cohort should have 6 starters, got {jan_3['started_funnel_users']}"
        assert jan_3['completed_funnel_users'] == 0, f"Jan 3 cohort should have 0 converters, got {jan_3['completed_funnel_users']}"
        assert abs(jan_3['conversion_rate'] - 0.0) < 0.01, f"Jan 3 conversion should be 0%, got {jan_3['conversion_rate']}"
        
        # Jan 4 should not exist as a cohort (no signups), even though conversions happened
        assert '2024-01-04' not in results_dict, "Jan 4 should not appear as a cohort since no signups occurred"
    ...
    def test_edge_case_same_minute_signup_conversion(self):
        ...
        assert result['completed_funnel_users'] == 1, f"Should have 1 converter, got {result['completed_funnel_users']}"
        expected_conversion = 50.0
        assert abs(result['conversion_rate'] - expected_conversion) < 0.01, f"Conversion should be {expected_conversion}%, got {result['conversion_rate']}"
