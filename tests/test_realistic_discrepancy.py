...
class TestRealWorldDiscrepancy:
    ...
    def test_realistic_discrepancy_scenario(self, problematic_data, calculator):
        ...
        print(
            f"- Time range: {problematic_data['timestamp'].min()} to {problematic_data['timestamp'].max()}"
        )

        step1_users = problematic_data[problematic_data["event_name"] == "step1"][
            "user_id"
        ].nunique()
        step2_users = problematic_data[problematic_data["event_name"] == "step2"][
            "user_id"
        ].nunique()
        ...
        if not timeseries_results.empty:
            ...
            for idx, row in top_periods.iterrows():
                print(
                    f"     {row['period_date']}: {row['conversion_rate']:.2f}% "
            ...
            if conversion_diff > 5:
                ...
                print(
                    "   - Overall funnel: Counts all users who ever did step1, checks if they later did step2"
                )
                print(
                    "   - Timeseries: Uses COHORT analysis - only counts users who STARTED in each period"
                )
                print(
                    "   - Users who start in one period but convert in another period may be counted differently"
                )
            else:
                print("   ✓ Methodologies are consistent")

            # Check if we have periods with very high conversion rates that could confuse users
            high_conversion_periods = timeseries_results[
                timeseries_results["conversion_rate"] > 50
            ]
            if not high_conversion_periods.empty:
                ...
                for idx, row in high_conversion_periods.iterrows():
                    print(
                        f"     {row['period_date']}: {row['conversion_rate']:.1f}% "
    ...
    def test_cohort_vs_overall_explanation(self, problematic_data, calculator):
        ...
        all_step1_users = set(
            problematic_data[problematic_data["event_name"] == "step1"]["user_id"]
        )
        all_step2_users = set(
            problematic_data[problematic_data["event_name"] == "step2"]["user_id"]
        )
