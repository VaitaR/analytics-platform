...
class TestCompleteIntegrationFlow:
    ...
    def integration_test_data(self, base_timestamp):
        ...
        for i in range(100):
            ...
            events.append({
                ...
                'event_properties': json.dumps({'source': 'organic', 'campaign': 'spring_2024'}),
                'user_properties': json.dumps({'segment': 'premium', 'country': 'US', 'age': 25 + (i % 20)})
            })
            ...
            if i < 90:
                events.append({
                    ...
                    'event_properties': json.dumps({'source': 'organic', 'method': 'email_click'}),
                    'user_properties': json.dumps({'segment': 'premium', 'country': 'US', 'age': 25 + (i % 20)})
                })
            ...
            if i < 60:
                events.append({
                    ...
                    'event_properties': json.dumps({'amount': 29.99 + (i % 10), 'currency': 'USD'}),
                    'user_properties': json.dumps({'segment': 'premium', 'country': 'US', 'age': 25 + (i % 20)})
                })
        ...
        for i in range(150):
            ...
            events.append({
                ...
                'user_properties': json.dumps({'segment': 'basic', 'country': 'UK', 'age': 30 + (i % 15)})
            })
            ...
            if i < 90:
                events.append({
                    ...
                    'event_properties': json.dumps({'source': 'paid', 'method': 'email_click'}),
                    'user_properties': json.dumps({'segment': 'basic', 'country': 'UK', 'age': 30 + (i % 15)})
                })
            ...
            if i < 30:
                events.append({
                    ...
                    'event_properties': json.dumps({'amount': 19.99 + (i % 5), 'currency': 'USD'}),
                    'user_properties': json.dumps({'segment': 'basic', 'country': 'UK', 'age': 30 + (i % 15)})
                })
    ...
    def test_complete_file_upload_to_results_flow(self, integration_test_data, expected_integration_results):
        """
        ...
        for event in expected_events:
            assert event in actual_events, f"Event '{event}' not found in loaded data. Found: {actual_events}"
    ...
    def test_segmentation_integration_flow(self, integration_test_data, expected_integration_results):
        """
    ...
    def test_reentry_mode_integration(self, base_timestamp):
        ...
        user_events = [
            ('Step_1', 0), ('Step_2', 30),  # First attempt - partial
            ('Step_1', 60), ('Step_2', 90), ('Step_3', 120),  # Second attempt - complete
        ]
    ...
    def test_error_handling_integration(self):
        ...
        invalid_data_scenarios = [
            ...
            pd.DataFrame({  # Invalid timestamp format
                'user_id': ['user1', 'user2'],
...
class TestDataSourceIntegration:
    ...
    def test_file_format_support_integration(self, integration_test_data):
        ...
        config = FunnelConfig()
        calculator = FunnelCalculator(config)
        results = calculator.calculate_funnel_metrics(loaded_csv, ['Sign Up', 'Email Verification'])
    ...
    def test_data_preprocessing_integration(self, base_timestamp):
        ...
        for i in range(25):  # Half the users complete a second event
            events.append({
                ...
                'user_properties': json.dumps({'age': 25 + (i % 30), 'country': 'US' if i % 3 == 0 else 'UK'})
            })
        
        test_data = pd.DataFrame(events)
        
        # Run funnel analysis with two steps
        config = FunnelConfig(segment_by='country' if 'country' in segmentation_props.get('user_properties', []) else None)
        calculator = FunnelCalculator(config)
