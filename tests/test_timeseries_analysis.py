...
class TestTimeSeriesCalculation:
            ...
            for user_id in range(daily_users):
                ...
                if np.random.random() < 0.7:
                    ...
                    if np.random.random() < 0.5:
                        profile_timestamp = verify_timestamp + timedelta(minutes=np.random.randint(10, 60))
                        events.append({
                            'user_id': f'user_{day}_{user_id}',
                        ...
                        if np.random.random() < 0.3:
                            ...
                            purchase_timestamp = profile_timestamp + timedelta(hours=np.random.randint(1, 4))
                            events.append({
                                'user_id': f'user_{day}_{user_id}',
    ...
    def test_timeseries_daily_aggregation(self, calculator, sample_events_data, funnel_steps):
        ...
        assert 'completed_funnel_users' in result.columns, "Should have completed_funnel_users column"
        ...
        # Validate data types
        assert pd.api.types.is_datetime64_any_dtype(result['period_date']), "period_date should be datetime"
        assert pd.api.types.is_integer_dtype(result['started_funnel_users']), "started_funnel_users should be integer"
        assert pd.api.types.is_numeric_dtype(result['conversion_rate']), "conversion_rate should be numeric"
...
class TestTimeSeriesIntegration:
    ...
    def test_timeseries_performance_monitoring(self):
        ...
        timeseries_functions = [func for func in final_metrics.keys() 
                              if 'timeseries' in func.lower()]
...
class TestTimeSeriesPerformance:
    ...
    def test_large_dataset_performance(self):
        ...
        result = calculator.calculate_timeseries_metrics(
            large_df,
            ['Step1', 'Step2'],  # Use 2 steps for a proper funnel
            '1d'
        )
        ...
        assert execution_time < 10.0, f"Should complete in under 10 seconds, took {execution_time:.2f}s"
        
        print(f"✅ Large dataset performance test passed: {execution_time:.2f}s for {len(large_df)} events")
