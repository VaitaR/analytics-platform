    ...
    class PlFunnelTest:
        ...
        def test_between_steps_events(self):
            ...
            for i in range(len(funnel_steps)-1):
                ...
                step_users = pl_funnel_events.filter(pl.col('event_name') == step)['user_id'].unique().to_list()
                next_step_users = pl_funnel_events.filter(pl.col('event_name') == next_step)['user_id'].unique().to_list()
                ...
                for user_id in common_users:
                    ...
                    if user_step_events.height > 0 and user_next_step_events.height > 0:
                        ...
                        next_step_time = user_next_step_events.select(pl.col('timestamp').min()).item()
                ...
                if between_event_counts:
                    sorted_events = sorted(
                        between_event_counts.items(), 
                        key=lambda x: x[1], 
                        reverse=True
                    )
                    step_pairs.append({
                        'step_pair': step_pair_key,
                        'top_events': dict(sorted_events)
                    })
