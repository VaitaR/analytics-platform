...
def generate_large_test_data(num_users=2000, events_per_user=15):
    ...
    events = []
    event_names = [
        "login", "view_homepage", "search", "view_product", "add_to_cart", 
        "view_cart", "checkout_start", "payment_info", "purchase", "logout",
        "browse_category", "view_reviews", "compare_products", "wishlist_add", 
        "error_page", "timeout", "back_button", "refresh_page", "contact_support"
    ]
    now = datetime.now()
...
def scalability_test():
    ...
    print(f"Target: >10,000 events/sec for production use")
    print(f"Acceptable: >5,000 events/sec for most use cases")
    ...
    for result in results:
        ...
        print(f"{result['users']:>4} users ({result['events']:>5,} events): {result['events_per_sec']:>6,.0f} events/sec - {status}")
