...
def benchmark_process_mining():
    ...
    for num_users, events_per_user in test_sizes:
        ...
        print(
            f"Generated {total_events:,} events for {unique_users:,} users in {data_gen_time:.2f}s"
        )
        ...
        result_no_cycles = analyzer.discover_process_mining_structure(
            df, min_frequency=5, include_cycles=False
        )
        ...
        result_with_cycles = analyzer.discover_process_mining_structure(
            df, min_frequency=5, include_cycles=True
        )
    ...
    for r in results:
        ...
        print(
            f"{size_label:<10} {events_label:<8} {no_cycles_label:<12} {with_cycles_label:<12} {overhead_label:<10}"
        )
    ...
    for r in results:
        ...
        if r["events_per_sec_no_cycles"] >= target_events_per_sec:
            print(
                f"  ✅ No cycles: {r['events_per_sec_no_cycles']:,.0f} events/sec (meets target)"
            )
        else:
            print(
                f"  ❌ No cycles: {r['events_per_sec_no_cycles']:,.0f} events/sec (below target)"
            )

        if r["events_per_sec_with_cycles"] >= target_events_per_sec:
            print(
                f"  ✅ With cycles: {r['events_per_sec_with_cycles']:,.0f} events/sec (meets target)"
            )
        else:
            print(
                f"  ❌ With cycles: {r['events_per_sec_with_cycles']:,.0f} events/sec (below target)"
            )
