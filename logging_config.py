...
def log_dataframe_info(df, name: str = "DataFrame", logger=None):
    ...
    try:
        ...
        # Sample data
        if len(df) > 0:
            logger.debug(f"   Sample row: {df.head(1).to_dict('records')[0] if hasattr(df, 'to_dict') else 'N/A'}")
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if 'struct' in error_msg and 'fields' in error_msg:
                logger.error("   🔍 This appears to be a Polars struct.fields() API compatibility issue")
